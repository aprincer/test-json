...
func (c *CorridaServiceImpl) FindBySondajeId(ctx context.Context, sondajeId int) []response.CorridaResponse {
	corridas := c.CorridaRepository.FindBySondajeId(ctx, sondajeId)
...
func (c *CorridaServiceImpl) FindById(ctx context.Context, corridaId int) response.CorridaResponse {
	value, err := c.CorridaRepository.FindById(ctx, corridaId)
	helper.PanicIfError(err)
	corridaResp := response.CorridaResponse{
		//Col_name: value.Col_name, Data_type: value.Data_type,
		Id:                         value.Id,
		Id_sondaje:                 value.Id_sondaje,
		Secuencia:                  value.Secuencia,
