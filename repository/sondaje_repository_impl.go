...
func (s *SondajeRepositoryImp) FindById(ctx context.Context, sondajeId int) (model.Sondaje, error) {
	tx, err := s.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)

	SQL := "SELECT * FROM sondaje s INNER JOIN corrida c ON s.id=c.id_sondaje WHERE s.id=$1"

	result, errQuery := tx.QueryContext(ctx, SQL, sondajeId)
	...
	for result.Next() {
		...
		return sondaje, errors.New("Sondaje not Found!")
	}*/
	var sondTmp []model.Sondaje
	...
	if sondTmp != nil {
		...
		return data, errors.New("sondaje not found")
	}
}
...
func (s *SondajeRepositoryImp) FindAll(ctx context.Context) []model.Sondaje {
	...
	SQL := "SELECT * FROM sondaje s INNER JOIN corrida c ON s.id=c.id_sondaje"
	...
	for result.Next() {
		...
		//fmt.Printf("SondajeId:%s", sondaje.Nombre_sondaje)
		//Añadimos sondaje al Mapa
		if _, ok := sondajes[sondaje.Id]; !ok {
			sondajes[sondaje.Id] = &model.Sondaje{
				Id:             sondaje.Id,
