...
func (c *CorridaRepositoryImpl) FindBySondajeId(ctx context.Context, sondajeId int) []model.Corrida {
	tx, err := c.Db.Begin()
	helper.PanicIfError(err)
	defer helper.CommitOrRollback(tx)
	...
	SQL := "SELECT * FROM corrida c LEFT JOIN fractura f ON c.id=f.id_corrida WHERE c.id_sondaje=$1 ORDER BY c.secuencia ASC"
...
func (c *CorridaRepositoryImpl) FindById(ctx context.Context, corridaID int) (model.Corrida, error) {
	...
	SQL := "SELECT * FROM corrida c LEFT JOIN fractura f ON c.id=f.id_corrida WHERE c.id=$1"
	...
	if data != nil {
		...
		cr := model.Corrida{}
		return cr, errors.New("corrida not found")
	}
